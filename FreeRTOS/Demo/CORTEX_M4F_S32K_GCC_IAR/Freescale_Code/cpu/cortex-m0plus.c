 ...
 *
 * File:        arm_cm0plus.c
 * Purpose:     Generic high-level routines for ARM Cortex M0+ processors
 *
 * Notes:
 */
...
void enable_irq (int irq)
{
    ...
    {
        case 0x0:
            S32_NVIC->ICPR[0] = 1 << (irq%32);
            S32_NVIC->ISER[0] = 1 << (irq%32);
            break;
        default:
            break;
    }
...
void set_irq_priority (int irq, int prio)
{
    ...
    prio_reg = (uint8 *)(((uint32)&S32_NVIC->IPR[0]) + irq);
