...
/*
 *
 * main.c (this file) defines a very simple demo that creates two tasks,
 * one queue. *
 *
 ...
 * if the value equals the expected 100, toggles the PIN.  The 'block
 ...
 * Blocked state every 200 milliseconds, and therefore toggles the PIN
...
volatile unsigned int SendTask_Tick = 0;
volatile unsigned int ReceiveTask_Tick = 0;
...
unsigned long ulReceivedValue;
	...
	for( ;; )
	{
		...
		{
			if ( REG_READ32(SIUL2_GPDOn( 9 )) == 0x01000000 )
				REG_WRITE32(SIUL2_GPDOn( 9 ), 0 << 24);
			else
				REG_WRITE32(SIUL2_GPDOn( 9 ), 1 << 24);
		}
