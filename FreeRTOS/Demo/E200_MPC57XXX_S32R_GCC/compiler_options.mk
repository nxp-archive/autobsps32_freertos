...
ASFLAGS  :=  -g --gdwarf-sections --gstabs+ -L -mregnames -mvle -I$(shell cygpath -m -i $(CRT_DIR))/SOURCES -I$(shell cygpath -m -i $(CRT_DIR))/SOURCES_HARDWARE/$(MCU) \
			 -I$(shell cygpath -m -i $(OS_ROOT))/Source/portable/GCC/PowerPC

CPPFLAGS  := -g -L -mregnames -mvle -I$(shell cygpath -m -i $(CRT_DIR))/SOURCES -I$(shell cygpath -m -i $(CRT_DIR))/SOURCES_HARDWARE/$(MCU) \
			 -I$(shell cygpath -m -i $(OS_ROOT))/Source/portable/GCC/PowerPC \
...
CFLAGS   :=  --sysroot=$(shell cygpath -m -i $(COMPILER_LIBS)) \
			...
			-I$(shell cygpath -m -i $(CRT_DIR))/SOURCES_HARDWARE/$(MCU)\
...
LDFLAGS :=  --sysroot=$(shell cygpath -m -i $(COMPILER_LIBS)) -mcpu=$(CPU) -mfpu=sp_full -Xlinker -Map=$(shell cygpath -m -i $(OUT_ROOT)/$(TEST_NAME).map) \
			-nostartfiles -T$(shell cygpath -m -i $(CRT_DIR))/LINKERSCRIPTS/flash_$(MCU).ld -static -Xlinker --gc-sections -Xlinker --print-gc-sections
