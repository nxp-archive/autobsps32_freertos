...
void OS_InstallInterruptHandler(OSInterruptHandlerPtr handlerFn, unsigned int vectorNum, unsigned char Priority)
{
    ...
    HWINTC_PSR(vectorNum) = (short unsigned int)((short unsigned int)(HWINTC_SET_PRC_SEL) | (short unsigned int)Priority);
...
void OS_PlatformInit(void)
{
    ...
    HWINTC_MCR = HWINTC_MCR & HWINTC_MCR_HVEN_SW; /* clear HVEN - software mode */
